...
;(function ($, window, document, undefined) {
    ...
    let startHour = 1;
    const url = $('#user-panel').attr('data-workstart-url');
    $.ajax({
        url: url,
        format: 'json',
        success: data => {
    ...
    $.extend(Plugin.prototype, {
        ...
        init: function () {
            ...
            this.periodPosition = 140 / this.periodInterval;
        ...
        create: function () {
            ...
            const url = $('#user-panel').attr('data-url');
            $.ajax({
                url: url,
                data: {},
                success: function (data) {
                    for (var j = data.work_from; j <= data.work_to; j++) {
                        $('<div class="jqs-grid-line"><div class="jqs-grid-hour">' + this_obj.formatHour(j) + '</div></div>').appendTo($('.jqs-grid', this_obj.element));
                    }
                    const height = (data.work_to - data.work_from) * 142;
                    $('.jqs-day').css('height', `${height}px`);
                }
            });
            ...
            for (var k = 0; k < this.settings.days; k++) {
                ...
                $('<div class="jqs-grid-day">' + day + dayRemove + dayDuplicate + '</div>').appendTo($('.jqs-grid-head', this.element));
            }
        },
        ...
        add: function (parent, position, height, options) {
            ...
            const top = (position * (this.periodPosition + 0.5)) - (startHour * (this.periodPosition + 0.5));
            if (top >= 0) {
